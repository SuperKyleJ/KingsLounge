...
export async function fetchShishaEntities() {
  const url = `${BASE_URL}/records/query?ckAPIToken=${API_TOKEN}`;

  const body = {
    query: {
      recordType: 'FlavorModel',
filterBy: [
      {
        fieldName: 'title',
...
const VALID_CATEGORIES = ['regular', 'exotic', 'limitedEdition'];

return data.records.map((record) => {
  const rawCategory = record.fields?.category?.value;
  const category = VALID_CATEGORIES.includes(rawCategory) ? rawCategory : 'regular';

  return {
    id: record.recordName,
    title: record.fields?.title?.value ?? '',
    backgroundColor: record.fields?.backgroundColor?.value ?? '',
    foregroundColor: record.fields?.foregroundColor?.value ?? '',
